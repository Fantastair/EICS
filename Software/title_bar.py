...
class TitleBar(fantas.Label):
    ...
    def __init__(self):
        super().__init__((u.window.size[0] * u.dpi_ratio, TitleBar.HEIGHT), bg=colors.THEMEBLUE, topleft=(0, 0))
        self.anchor = 'topleft'
    ...
    def auto_set_width(self):
        if u.window.size[0] * u.dpi_ratio != self.rect.w:
            self.set_size((u.window.size[0] * u.dpi_ratio, self.rect.h))
        ...
        elif u.window.size[1] * u.dpi_ratio != self.last_window_height:
            for i in self.page_buttons:
                self.page_buttons[i].auto_adjust_size()
        self.last_window_height = u.window.size[1] * u.dpi_ratio
    ...
    def get_usable_rect(self):
        return pygame.Rect(connect_bar.ConnectBar.WIDTH, TitleBar.HEIGHT, u.window.size[0] * u.dpi_ratio - connect_bar.ConnectBar.WIDTH, u.window.size[1] * u.dpi_ratio - TitleBar.HEIGHT)
...
class TitleBarWidget(fantas.MouseBase):
    ...
    def mousemove(self, pos):
        ...
        if self.start_pos is not None:
            ...
            on_right = pos[0] >= u.window.size[0] * u.dpi_ratio - self.EDGE
            on_bottom = pos[1] >= u.window.size[1] * u.dpi_ratio - self.EDGE
            if on_left:
                if on_top:
