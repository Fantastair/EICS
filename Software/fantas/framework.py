...
class UiManager:
    ...
    def init(self, title: str, size, **kwargs):
        # 初始化（创建窗口等）
        self.clock = pygame.time.Clock()
        self.size = size
        self.window = pygame.Window(title, size, **kwargs)
...
def get_mouse_position_on_screen():
    p = pygame.mouse.get_pos()
    return p[0] + uimanager.window.position[0], p[1] + uimanager.window.position[1]
