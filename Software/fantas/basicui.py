...
class Animation(fantas.Ui):
    def __init__(self, img_path, **anchor):
        self.img_queue = pygame.image.load_animation(img_path)
        self.length = len(self.img_queue)
        self.loop = 0
        self.current_frame = 0
        for i in range(self.length):
            self.img_queue[i] = (self.img_queue[i][0].convert_alpha(), self.img_queue[i][1])
        super().__init__(self.img_queue[0][0], **anchor)
        self.keyframe = fantas.AnimationKeyFrame(self)
        self.stop_callback = None
        self.stop_callback_args = None
        self.stop_callback_kwargs = None

    def set_frame(self, frame):
        self.current_frame = frame
        self.img = self.img_queue[frame][0]
        self.mark_update()
    ...
    def play(self, loop=0):
        self.loop = loop
        self.keyframe.launch()

    def stop(self):
        self.keyframe.stop()
        if self.stop_callback is not None:
            self.stop_callback(*self.stop_callback_args, **self.stop_callback_kwargs)

    def bind_stop_callback(self, func, *args, **kwargs):
        self.stop_callback = func
        self.stop_callback_args = args
        self.stop_callback_kwargs = kwargs
...
class CircleLabel(Label):
    ...
    def update_img(self):
        self.img = pygame.Surface((self.radius_*2 + 1, self.radius_*2 + 1), flags=pygame.SRCALPHA)
